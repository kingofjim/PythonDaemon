...
class Database:
    ...
    def get_current_dns_record(self, year_month, date, hour):
        if not self.check_table_exist('cdn_dns_logs_%s' % year_month):
            ...
            return {}
        cur = self.logs.cursor()
        query = 'select id, query, domain from cdn_dns_logs_%s where date="%s" and hour=%s;' % (year_month, date, hour)
        cur.execute(query)
        result = {}
